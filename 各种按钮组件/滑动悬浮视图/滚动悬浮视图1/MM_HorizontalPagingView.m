...
@interface MM_HorizontalPagingView()<UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation MM_HorizontalPagingView

static void *MMHorizontalPagingViewScrollContext = &MMHorizontalPagingViewScrollContext;
static void *MMHorizontalPagingViewInsetContext = &MMHorizontalPagingViewInsetContext;
static void *MMHorizontalPagingViewPanContext = &MMHorizontalPagingViewPanContext;
...
+(MM_HorizontalPagingView *)pagingViewWithHeaderView:(UIView *)headerView
    ...
    MM_HorizontalPagingView *pagingView = [[MM_HorizontalPagingView alloc] initWithFrame:CGRectMake(0.f, 64.f, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
...
-(void)configureHeaderView{
    if (self.headerView) {
        ...
        [self addConstraint:[NSLayoutConstraint constraintWithItem:self.headerView
                                                        ...
                                                        multiplier:1
                                                          constant:0]];
        self.headerOriginYConstraint = [NSLayoutConstraint constraintWithItem:self.headerView
...
//        [self addConstraint:[NSLayoutConstraint constraintWithItem:self.headerView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeBottom multiplier:1 constant:0]];
...
-(void)configureContentView{
    for (UIScrollView *v in self.contentViews) {
        NSLog(@"%d",(int)(self.headerViewHeight + self.segmentBarHeight));
        ...
        //v.contentOffset = CGPointMake(0.f,-self.headerViewHeight - self.segmentBarHeight);
        [v.panGestureRecognizer addObserver:self forKeyPath:NSStringFromSelector(@selector(state)) options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:&MMHorizontalPagingViewPanContext];
        [v addObserver:self forKeyPath:NSStringFromSelector(@selector(contentOffset)) options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:&MMHorizontalPagingViewScrollContext];
        [v addObserver:self forKeyPath:NSStringFromSelector(@selector(contentInset)) options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:&MMHorizontalPagingViewInsetContext];
    }
...
-(void)adjustContentViewOffset{
    ...
    NSLog(@"self.currentScrollview.contentOffset.y = %f,\t -self.segmentHeitht = %f",self.currentScrollView.contentOffset.y,-self.segmentBarHeight);
...
//    NSLog(@"地址MMHorizontalPagingViewPanContext = %p",MMHorizontalPagingViewPanContext);
    if (context == &MMHorizontalPagingViewPanContext) {
        self.horizontalCollectionView.scrollEnabled = YES;
    ...
    }else if(context == &MMHorizontalPagingViewScrollContext){
        self.currentTouchView = nil;
        self.currentTouchButton = nil;
        if (self.isSwitching) {
            return;
...
//        }
    ...
    }else if(context == &MMHorizontalPagingViewInsetContext){
        if (self.currentScrollView.contentOffset.y > -self.segmentBarHeight) {
            return;
        }
...
-(void)dealloc{
    for (UIScrollView *v in self.contentViews) {
        [v.panGestureRecognizer removeObserver:self forKeyPath:NSStringFromSelector(@selector(state)) context:&MMHorizontalPagingViewPanContext];
        [v removeObserver:self forKeyPath:NSStringFromSelector(@selector(contentOffset)) context:&MMHorizontalPagingViewScrollContext];
        [v removeObserver:self forKeyPath:NSStringFromSelector(@selector(contentInset)) context:&MMHorizontalPagingViewInsetContext];
    }
}
